// gpio_shared_button_state.rs - Stops the program until a certain amount of
// event input changes via a non-global shared variable (that can be done using
// OnceCell for example). This requires a Mutex as it goes across threads, and
// Arc to make sure we have the same entry everywhere.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // We need to clone this as set_async_interrupt will move it, which means it
    ...
    loop {
        ...
        // Suppose we do some work here that takes a second. The shorter the
        // work takes, the quicker we will quit upon reaching our
        // condition.
